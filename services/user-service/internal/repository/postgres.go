...
func (r *PostgresRepository) CreateProfile(ctx context.Context, p *domain.UserProfile) error {
	query := `
		INSERT INTO profiles (id, display_name, avatar_url, bio, custom_status, preferred_language)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING created_at, updated_at
	`
	return r.db.QueryRowContext(ctx, query, p.ID, p.DisplayName, p.AvatarURL, p.Bio, p.CustomStatus, p.PreferredLanguage).
		Scan(&p.CreatedAt, &p.UpdatedAt)
}

func (r *PostgresRepository) GetProfileByID(ctx context.Context, id string) (*domain.UserProfile, error) {
	query := `SELECT id, display_name, avatar_url, bio, custom_status, preferred_language, created_at, updated_at FROM profiles WHERE id = $1`
	p := &domain.UserProfile{}
	err := r.db.QueryRowContext(ctx, query, id).
		Scan(&p.ID, &p.DisplayName, &p.AvatarURL, &p.Bio, &p.CustomStatus, &p.PreferredLanguage, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
...
func (r *PostgresRepository) UpdateProfile(ctx context.Context, p *domain.UserProfile) error {
	query := `
		UPDATE profiles 
		SET display_name = $1, bio = $2, avatar_url = $3, custom_status = $4, preferred_language = $5, updated_at = NOW()
		WHERE id = $6
	`
	_, err := r.db.ExecContext(ctx, query, p.DisplayName, p.Bio, p.AvatarURL, p.CustomStatus, p.PreferredLanguage, p.ID)
	return err
}
